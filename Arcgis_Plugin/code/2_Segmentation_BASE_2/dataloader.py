...
class RS_Loader_Semantic(data.Dataset):
    def __init__(self, dataset_path, num_classes, mode, indices, isRGB, transform=None, memory_load=False):
        ...
        self.class_weights = self.get_distribution()
        '''
        print("Distribution")
    ...
    def get_distribution(self):
        ...
        # Compute Weight for class in segsemantic
        for index, mask_path in enumerate(self.list_labels):
            ...
            for l, c in zip(labels, count):
                if l != 254.0:
                    class_weights[int(l)] += c
                
                
        if True:
            # 1) Normalize distribution over proportion
            print('Normalize distribution over proportion')
            weight_sum = sum(class_weights)/class_weights
            weight_sum_norm = weight_sum/sum(weight_sum)
            class_weights = weight_sum_norm
            print('{} Class Weight: {}'.format(self.mode, class_weights))
        else:
            # 1) Normalize using Inverse of #samples
            print('Normalize Inverse of proportion sampler')
        ...
        if not self.memory_load:
            ...
            try:
                ...
                if not self.isRGB:
                    ...
                    img = np.concatenate((img_tmp, dec_tmp, mdt_tmp), axis=2)
